...
```{r }
dataH = subset.data.frame(data, Sexo == "0" | Sexo == "2")
dataM = subset.data.frame(data, Sexo == "1" | Sexo == "2")

```

 Se pudo observar, a partir de pruebas, que no exite una variable en especifico que tenga una alta correlacion con el precio, sin embargo hubo variables como la duracion y habitaciones disponibles que si tenian un  considerable nivel de correlacion por lo que se utilizó una regresión múltiple, haciendo uso de las variables que se consideraron útiles para asi obtener una mejor predicción.
